...
export default async function ContactDetailPage({ params }) {
  ...
  const [
    ...
    supabase.from('tasks').select('*').eq('contact_name', (await supabase.from('contacts').select('name').eq('id', id).single()).data?.name || '').order('created_at', { ascending: false }),
    ...
    supabase.from('messages').select('*').eq('contact_name', (await supabase.from('contacts').select('name').eq('id', id).single()).data?.name || '').order('sent_at', { ascending: false }).limit(10),
  ]);
