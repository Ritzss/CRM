...
const TEMPLATES = {
  ...
  delayed:   { label: 'Delayed',          body: n => `Hi ${n}, we're sorry — your delivery has been slightly delayed. We'll notify you as soon as it ships.` },
...
export default function MessagesClient({ initialContacts, initialHistory }) {
  ...
  async function send() {
    ...
    try {
      const endpoint = form.channel === 'sms' ? '/api/send-sms' : '/api/send-email';
      const payload  = form.channel === 'sms'
        ? { to: contact.phone, body: form.body }
        : { to: contact.email, subject: 'Delivery Update', body: form.body };

      if (!contact.phone && form.channel === 'sms') throw new Error('This contact has no phone number.');
      if (!contact.email && form.channel === 'email') throw new Error('This contact has no email address.');
      ...
      await db.from('messages').insert([{ contact_name: contact.name, channel: form.channel, body: form.body }]);
    ...
    <div className="grid gap-5" style={{ gridTemplateColumns: '1fr 320px' }}>
      <div className="bg-white border border-gray-100 rounded-xl p-6">
        ...
        <div className="mb-4">
          ...
          <select className={inputCls} value={form.contactId} onChange={e => setForm({ ...form, contactId: e.target.value })}>
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            {[['email', 'Email (SendGrid)'], ['sms', 'SMS (Twilio)']].map(([ch, lbl]) => (
              <button key={ch} type="button" onClick={() => setForm({ ...form, channel: ch })}
                className={`px-4 py-2 text-[13px] rounded-lg border transition-all cursor-pointer
                  ${form.channel === ch ? 'border-emerald-400 bg-emerald-50 text-emerald-700 font-semibold' : 'border-gray-200 bg-white text-gray-500 hover:bg-gray-50'}`}>
                {lbl}
              </button>
            ))}
          </div>
        ...
        <div className="mb-4">
          ...
          <textarea className={inputCls} value={form.body} onChange={e => setForm({ ...form, body: e.target.value })}
        ...
        {alert.msg && (
          <div className={`px-4 py-2.5 rounded-lg text-[13px] mb-4 border
            ${alert.type === 'ok' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-600 border-red-200'}`}>
            {alert.msg}
          </div>
        )}
          ...
          : history.map(m => (
            <div key={m.id} className="border-b border-gray-50 pb-3 mb-3 last:border-0 last:mb-0">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${m.channel === 'sms' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>
